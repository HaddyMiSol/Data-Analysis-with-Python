...
folder_path = 'C:/Users/DELL/Documents/DataFi/Data Review Meeting/CS/CS_RADET_Files/client_updated2'

# Output path for the final Excel file
output_file_path = 'C:/Users/DELL/Documents/DataFi/Data Review Meeting/CS/FY26Q1_Aggregate_data_client_____CARE2.xlsx'

# Path for the separate viral load output file
viral_load_output_path = 'C:/Users/DELL/Documents/DataFi/Data Review Meeting/CS/Cleaned_Viral_Load_Values_IP__.xlsx'

# Path for the unique CD4 values output file
unique_cd4_output_path = 'C:/Users/DELL/Documents/DataFi/Data Review Meeting/CS/Cleaned_CD4_Values_IP__.xlsx'
...
required_cols_for_aggregation = ['ProjectName', 'Facility Name',  ]
...
# Defining Periods
Start_of_quarter = pd.to_datetime('2025-10-01')
End_of_quarter = pd.to_datetime('2025-12-31')
vl_start = End_of_quarter - pd.DateOffset(months=12, days=-1) #12 months from the end of the quarter
six_months_ago = pd.to_datetime('2025-07-01')
End_of_vl_month = Start_of_quarter + pd.DateOffset(months=4, days=-1) #taking into consideration the one month of result recieved for vl samples collected in the last quarter


# End_of_quarter = pd.to_datetime('2025-08-22')
# vl_start = End_of_quarter - pd.DateOffset(months=12, days=-1) #12 months from the end of the quarter
# # six_months_ago = pd.to_datetime('2025-01-01')
# End_of_vl_month = End_of_quarter #Start_of_quarter + pd.DateOffset(months=4, days=-1) #taking into consideration the one month of result recieved for vl samples collected in the last quarter
...
# --- NEW: Updated Viral Load Cleaning Function ---
...
# # tx_curr_invalid = combined_data[
# #     (combined_data['Current ART Status'].isin(['Active', 'Active Restart'])) &
# #     ((combined_data['Client Verification Outcome'].isin(['valid', 'valid ', ' valid', 'Valid']))) &  
# #     (~combined_data['Sex'].isna())
# # ]

# # tx_curr_Verification_ongoing = combined_data[
# #     (combined_data['Current ART Status'].isin(['Active', 'Active Restart'])) &
# #     ((combined_data['Client Verification Outcome'].isin(['valid', 'valid ', ' valid', 'Valid'])))  &
# #     (~combined_data['Sex'].isna())
# # ]
...
# End_of_week = pd.to_datetime('2025-09-05')
# Time_on_ART_lt_three = (combined_data[
#       (combined_data['Current ART Status'].isin(['Active', 'Active Restart'])) &
#      ((combined_data['Client Verification Outcome'].isna()) | # Catches np.nan
#         (combined_data['Client Verification Outcome'] == '') | # Catches explicit empty strings
#         (combined_data['Client Verification Outcome'].str.strip() == '') ) &
#     ((End_of_week - combined_data['ART Start Date (yyyy-mm-dd)']).dt.days < 90)
#     ])

# Time_on_ART_gt_three_lt_five = (combined_data[
#       (combined_data['Current ART Status'].isin(['Active', 'Active Restart'])) &
#      ((combined_data['Client Verification Outcome'].isna()) | # Catches np.nan
#         (combined_data['Client Verification Outcome'] == '') | # Catches explicit empty strings
#         (combined_data['Client Verification Outcome'].str.strip() == '')) &
#     (((End_of_week - combined_data['ART Start Date (yyyy-mm-dd)']).dt.days >= 90) &
#     ((End_of_week - combined_data['ART Start Date (yyyy-mm-dd)']).dt.days <= 179))
#     ])


# Time_on_ART_gt_six = (combined_data[
#       (combined_data['Current ART Status'].isin(['Active', 'Active Restart'])) &
#      ((combined_data['Client Verification Outcome'].isna()) | # Catches np.nan
#         (combined_data['Client Verification Outcome'] == '') | # Catches explicit empty strings
#         (combined_data['Client Verification Outcome'].str.strip() == '')) &
#     (((End_of_week - combined_data['ART Start Date (yyyy-mm-dd)']).dt.days >= 180))
#     ])
...
# --- START OF ORIGINAL CODE WITH THE CHANGE TO USE THE NEW COLUMN ---
...
def grt_cd4(value):
    ...
    try:
        ...
        return False
# --- END OF ORIGINAL CODE WITH THE CHANGE TO USE THE NEW COLUMN ---
...
# # # common_condition_tx_rtt = (
# # #     (combined_data['Current ART Status'].isin(['Active Restart'])) &
# # #     ((combined_data['Date of Current ART Status'] >= Start_of_quarter) & (combined_data['Date of Current ART Status'] <= End_of_quarter)) &
# # #     ((combined_data['Previous ART Status'].str.contains('IIT', 'Stopped Treatment')) & #'Invalid - Long-term IIT',
# # #     (combined_data['Confirmed Date of Previous ART Status'] < Start_of_quarter) &
# # #     (combined_data['Client Verification Outcome'].isin(['valid', 'valid ', ' valid', 'Valid']))) &
# # #     (~combined_data['Sex'].isna()))
    
# # # firstt_condition = ((combined_data['Age'] >4) & (~combined_data['Last CD4 Count'].isna()) & (combined_data['Last CD4 Count'].apply(is_valid_cd4)) & ((combined_data['Date of Last CD4 Count'] >= Start_of_quarter) & (combined_data['Date of Last CD4 Count'] <= End_of_quarter)))
# # # secondd_condition = ((combined_data['Age'] <5))
# # # third_condition = ((combined_data['Age'] >4) & ((combined_data['Last CD4 Count'].isna()) | (combined_data['Last CD4 Count'].apply(is_valid_cd4)) | ((combined_data['Date of Last CD4 Count'] < Start_of_quarter))))
                   


# # # tx_rtt_first_condition = combined_data[common_condition_tx_rtt & firstt_condition]
# # # tx_rtt_second_condition = combined_data[common_condition_tx_rtt & secondd_condition]
# # # tx_rtt_third_condition = combined_data[common_condition_tx_rtt & third_condition]   
# # # tx_rtt = combined_data[common_condition_tx_rtt & (firstt_condition | secondd_condition | third_condition) ]

# # Filters for TX_RTT(iit<3)
# # Filters for TX_RTT(iit3-5)
# # Filters for TX_RTT(iit>=6)
# iit_lt_three = (
#     (combined_data['Date of Current ART Status'] - combined_data['Confirmed Date of Previous ART Status']).dt.days < 90)

# iit_btwn_three_to_five = (
#     ((combined_data['Date of Current ART Status'] - combined_data['Confirmed Date of Previous ART Status']).dt.days >= 90) &
#     ((combined_data['Date of Current ART Status'] - combined_data['Confirmed Date of Previous ART Status']).dt.days <= 179))

# iit_gt_six = (
#     (combined_data['Date of Current ART Status'] - combined_data['Confirmed Date of Previous ART Status']).dt.days >= 180)


# # tx_rtt_iit_lt_three = combined_data[(temp_tx_rtt) & (iit_lt_three)]
# # tx_rtt_btwn_three_to_five = combined_data[(temp_tx_rtt) & (iit_btwn_three_to_five)]
# # tx_rtt_iit_gt_six =combined_data[(temp_tx_rtt) & (iit_gt_six)]
...
# # # # Filters for Client Verification Outcome (Invalid)
# # # Invalid = combined_data[
# # #     (combined_data['Client Verification Outcome'].fillna('').str.contains('invalid')) &
# # #      ((combined_data['Date of Current ART Status'] >= Start_of_quarter) &
# # #     (combined_data['Date of Current ART Status'] <= End_of_quarter)) 
# # # ]

# # # # Filters for Client Verification Outcome (Verification ongoing)
# # # Verification_ongoing = combined_data[
# # #     (combined_data['Client Verification Outcome'].fillna('').str.contains('Verification Ongoing')) &
# # #      ((combined_data['Date of Current ART Status'] >= Start_of_quarter) &
# # #     (combined_data['Date of Current ART Status'] <= End_of_quarter)) 
# # # ]
...
# # Condition for 3HP for 'Date of TPT Start' being in July-Sept 2024 and 'TPT Completion date' in October-December 2024
# tb_prev_n_3HP_6mths =(        
...
#         (combined_data['TPT Completion status'].str.contains('Treatment Completed|Treatment success|Completed')))
        

# # Condition for 3HP for 'Date of TPT Start' being in October-December 2024 and 'TPT Completion date' in January-March 2025        
# tb_prev_n_3HP_3mths = (        
...
# # Condition for 3HP for 'Date of TPT Start' being in April-September 2024 and 'TPT Completion date' in October-March 2025        
# tb_prev_n_INH = (        
...
# # Condition for 'Date of TPT Start' being in October-December 2024
# tb_prev_d_3HP_3mths = (        
#         ((combined_data['Date of TPT Start (yyyy-mm-dd)'] >= threehp_period2_start) & #Jan 2025
#         (combined_data['Date of TPT Start (yyyy-mm-dd)'] <= threehp_period2_end)) & #Mar 2025
#         (combined_data['TPT Type'].str.contains('(3HP)|(3HR)')))
...
# # # print(combined_data['Client Verification Outcome'].dropna().str.strip().unique())
...
def pivot_data(data, value_name):
    ...
    for col in ['ProjectName', 'Facility Name','DatimId']:
        if col not in data.columns:
            ...
            return pd.DataFrame(columns=['ProjectName', 'Facility Name', 'DatimId', value_name]) # Return empty DataFrame 'Datim Id - for sync'
...
# tx_curr_invalid_pivot = pivot_data(tx_curr_invalid, 'TX_CURR_invalid')
# tx_curr_Verification_ongoing_pivot = pivot_data(tx_curr_Verification_ongoing, 'TX_CURR_Verification_Ongoing')
# tx_curr_valid_pivot = pivot_data(tx_curr_valid, 'TX_CURR_valid')
# tx_curr_blank_pivot = pivot_data(tx_curr_blank, 'TX_CURR_blank')
# Time_on_ART_lt_three_pivot = pivot_data(Time_on_ART_lt_three, 'Time_on_ART_lt_three')
# Time_on_ART_gt_three_lt_five_pivot = pivot_data(Time_on_ART_gt_three_lt_five, 'Time_on_ART_gt_three_lt_five')
# Time_on_ART_gt_six_pivot = pivot_data(Time_on_ART_gt_six, 'Time_on_ART_gt_six')
...
# tx_pvls_dd_pivot = pivot_data(tx_pvls_dd, 'TX_PVLS_Dd')
...
# # tx_new_first_condition_pivot = pivot_data(tx_new_first_condition, 'TX_NEW_first')
# # tx_new_second_condition_pivot = pivot_data(tx_new_second_condition, 'TX_NEW_second')
tx_new_pivot = pivot_data(tx_new, 'TX_NEW')
# transferin_pivot = pivot_data(TransferIn, 'Transferin')
# tx_new_Pregnant_and_BF_pivot = pivot_data(tx_new_Pregnant_and_BF, 'TX_NEW_Pregnant_and_BF')
...
all_pivots_for_summary = [
    tx_curr_pivot,
    # tx_curr_invalid_pivot,
    # tx_curr_Verification_ongoing_pivot,
    # tx_curr_valid_pivot,
    # tx_curr_blank_pivot,
    # Time_on_ART_lt_three_pivot,
    # Time_on_ART_gt_three_lt_five_pivot,
    # Time_on_ART_gt_six_pivot,
    ...
    # # tx_new_first_condition_pivot,
    # # tx_new_second_condition_pivot,
    tx_new_pivot,
    # transferin_pivot,
    # tx_new_Pregnant_and_BF_pivot,
    tx_new_BF_pivot,
    temp_tx_rtt_pivot,
    # tx_rtt_iit_lt_three_pivot,
    # tx_rtt_btwn_three_to_five_pivot,
    # tx_rtt_iit_gt_six_pivot,
    tx_ml_Stopped_pivot,
    tx_ml_Died_pivot,
    tx_ml_Transfer_out_pivot,
    tx_ml_IIT_pivot,
    # # # Invalid_pivot,
    # # verification_ongoing_pivot,
    ...
    # # tx_tb_d_Diagnosed_with_TB_pivot,
    # tx_tb_n_pivot,
    # tb_prev_n_pivot,
    # tb_prev_d_pivot
    ] 
...
# This applies to columns added from pivots (e.g., 'TX_CURR')
...
with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
    ...
    if master_aggregated_df.empty:
        ...
        for project in project_names:
            ...
            # and you prefer data to be by Facility Name within the sheet.
...
#         master_aggregated_df.sort_values(by=['ProjectName', 'Facility', 'DatimId'], inplace=True)

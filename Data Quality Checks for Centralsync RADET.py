...
folder_path = 'C:/Users/DELL/Documents/DataFi/Data Review Meeting/CS/IP_RADET_Filess'
...
for project_name, project_data in combined_data.groupby('ProjectName'):
    ...
    # Condition 7: Non-integer and Invalid Current Viral Load
  ...
  # Condition : Date of TB Screening < ART Start Date
    TBScreeningDate_lt_ARTStart = project_data[
        (project_data['Date of TB Screening (yyyy-mm-dd)'] < project_data['ART Start Date (yyyy-mm-dd)']) & (project_data['Client Verification Outcome'].isin(['valid', '']))]
    project_issues['TBScreeningDate_lt_ARTStart'] = TBScreeningDate_lt_ARTStart.shape[0]
    line_lists[f"{project_name}_TBScreeningDate_lt_ARTStart"] = TBScreeningDate_lt_ARTStart

    # Condition : Date of TB Screening with blank TB Screening Type
    ...
    line_lists[f"{project_name}_TBScreeningDate_without_TBScreeningType"] = TBScreeningDate_without_TBScreeningType

    # Condition : Date of TB Screening with blank TB Status
    ...
    line_lists[f"{project_name}_TBScreeningDate_without_TBStatus"] = TBScreeningDate_without_TBStatus


    # Condition : Date of TB Sample collection for Negative TB Status
    NegativeTBStatus_with_SampleCollectionDate = project_data[
        (~project_data['TB status'].isin(['Presumptive TB', 'Currently on TB treatment', 'Confirmed TB', ''])) & (project_data['Date of TB Sample Collection (yyyy-mm-dd)'].isna())]
    project_issues['NegativeTBStatus_with_SampleCollectionDate'] = NegativeTBStatus_with_SampleCollectionDate.shape[0]
    line_lists[f"{project_name}_NegativeTBStatus_with_SampleCollectionDate"] = NegativeTBStatus_with_SampleCollectionDate

    # Condition : Date of TB Screening with blank TB Diagnostic Test Type
    ...
    line_lists[f"{project_name}_TBSampleCollection_without_TBDiagnosticType"] = TBSampleCollection_without_TBDiagnosticType

    #Condition: Wrong format of TB Diagnostic Test Type
    Wrongformat_TBDiagnosticTestType = project_data[
        (~project_data['TB Diagnostic Test Type'].isin(['Gene Xpert', 'TB-LAM', 'LF-LAM', 'Chest X-ray']) | ~project_data['TB Diagnostic Test Type'].isna()) &
        (project_data['Client Verification Outcome'].isin(['valid', '']))]
    project_issues['Wrongformat_TBDiagnosticTestType'] = Wrongformat_TBDiagnosticTestType.shape[0]
    line_lists[f"{project_name}_Wrongformat_TBDiagnosticTestType"] = Wrongformat_TBDiagnosticTestType


    # Condition : Date of TB Sample Collection < ART start Date
    TBSampleCollectionDate_lt_ARTStart = project_data[
        (project_data['Date of TB Sample Collection (yyyy-mm-dd)'] < project_data['ART Start Date (yyyy-mm-dd)']) & (project_data['Client Verification Outcome'].isin(['valid', '']))]
    project_issues['TBSampleCollectionDate_lt_ARTStart'] = TBSampleCollectionDate_lt_ARTStart.shape[0]
    line_lists[f"{project_name}_TBSampleCollectionDate_lt_ARTStart"] = TBSampleCollectionDate_lt_ARTStart


    # Condition : Date of TB Diagnostic Result Received < ART start Date
    TBDiagnosticResultDate_lt_ARTStart = project_data[
        (project_data['Date of TB Diagnostic Result Received (yyyy-mm-dd)'] < project_data['ART Start Date (yyyy-mm-dd)']) & (project_data['Client Verification Outcome'].isin(['valid', '']))]
    project_issues['TBDiagnosticResultDate_lt_ARTStart'] = TBDiagnosticResultDate_lt_ARTStart.shape[0]
    line_lists[f"{project_name}_TBDiagnosticResultDate_lt_ARTStart"] = TBDiagnosticResultDate_lt_ARTStart

    # Condition : Wrong format of TB Diagnostic Result

    # Condition : Blank TB Diagnostic Result with Date of start of TB Treatment 
    DateStartofTBtreatment_without_TBDiagnosticResult = project_data[
        (project_data['TB Diagnostic Result'].isin(['MTB detected RIF resistance not detected', 'Positive', 'MTB detected RIF resistance detected', 'MTB Detected (Rifampicin not Resistance)', 'MTB DETECTED', 'MTB DETECTED RIF Resistance Indeterminate', 'MTB TRACE DETECTED RIF INDETERMINATE', 'MTB trace RIF resistance indeterminate'])) & 
        (project_data['Date of Start of TB Treatment (yyyy-mm-dd)'].isna()) &
        (project_data['Client Verification Outcome'].isin(['valid', '']))]
    project_issues['DateStartofTBtreatment_without_TBDiagnosticResult'] = DateStartofTBtreatment_without_TBDiagnosticResult.shape[0]
    line_lists[f"{project_name}_DateStartofTBtreatment_without_TBDiagnosticResult"] = DateStartofTBtreatment_without_TBDiagnosticResult


    # Condition :Date of start of TB Treatment where TB Diagnostic Result = 'Negative' or contains 'MTB not Detected
    DateStartofTBtreatment_where_TBDiagnosticResult_is_negative = project_data[
        (project_data['TB Diagnostic Result'].isin(['Negative', 'MTB not detected'])) & 
        (~project_data['Date of Start of TB Treatment (yyyy-mm-dd)'].isna()) &
        (project_data['Client Verification Outcome'].isin(['valid', ''])) ]
    project_issues['DateStartofTBtreatment_where_TBDiagnosticResult_is_negative '] = DateStartofTBtreatment_where_TBDiagnosticResult_is_negative .shape[0]
    line_lists[f"{project_name}_DateStartofTBtreatment_where_TBDiagnosticResult_is_negative "] = DateStartofTBtreatment_where_TBDiagnosticResult_is_negative 


    # Condition :Date of start of TB Treatment with blank TB Type (new, relapsed etc)
    DateStartofTBtreatment_without_TBTreatmentType = project_data[
        (~project_data['Date of Start of TB Treatment (yyyy-mm-dd)'].isna()) &
        (project_data['TB Treatment Type (new, relapsed etc)'].isna()) & 
        (project_data['Client Verification Outcome'].isin(['valid', ''])) ]
    project_issues['DateStartofTBtreatment_without_TBTreatmentType'] = DateStartofTBtreatment_without_TBTreatmentType.shape[0]
    line_lists[f"{project_name}_DateStartofTBtreatment_without_TBTreatmentType"] = DateStartofTBtreatment_without_TBTreatmentType


    # Condition : Date of Completion of TB Treatment < Date of Start of TB Treatment
    DateCompletionofTBtreatment_lt_DateStartofTBtreatment = project_data[
        (project_data['Date of Completion of TB Treatment (yyyy-mm-dd)'] < project_data['Date of Start of TB Treatment (yyyy-mm-dd)']) & (~project_data['Date of Start of TB Treatment (yyyy-mm-dd)'].isna()) & (project_data['Client Verification Outcome'].isin(['valid', '']))]
    project_issues['DateCompletionofTBtreatment_lt_DateStartofTBtreatment'] = DateCompletionofTBtreatment_lt_DateStartofTBtreatment.shape[0]
    line_lists[f"{project_name}_DateCompletionofTBtreatment_lt_DateStartofTBtreatment"] = DateCompletionofTBtreatment_lt_DateStartofTBtreatment


    # Condition :Date of Completion of TB Treatment with blank TB Treatment Outcome
    ...
    line_lists[f"{project_name}_DateCompletionofTBtreatment_without_TBtreatmentOutcome"] = DateCompletionofTBtreatment_without_TBtreatmentOutcome

    # Condition :Date of TPT Start where TB Status is positive
    DateTPTStart_where_TBStatus_is_positive = project_data[
        (project_data['TB status'].isin(['Presumptive TB', 'Currently on TB treatment', 'Confirmed TB'])) & 
        (~project_data['Date of TPT Start (yyyy-mm-dd)'].isna()) &
        (project_data['Client Verification Outcome'].isin(['valid', ''])) ]
    project_issues['DateTPTStart_where_TBStatus_is_positive'] = DateTPTStart_where_TBStatus_is_positive.shape[0]
    line_lists[f"{project_name}_DateTPTStart_where_TBStatus_is_positive"] = DateTPTStart_where_TBStatus_is_positive


    # Condition :Date of TPT Start with blank TPT Type
    ...
    line_lists[f"{project_name}_DateTPTStart_without_TPTtype"] = DateTPTStart_without_TPTtype


    # Condition : Date of TPT Completion < Date of TPT Start
    DateTPTcompletion_lt_DateTPTStart = project_data[
        (project_data['TPT Completion date (yyyy-mm-dd)'] < project_data['Date of TPT Start (yyyy-mm-dd)']) & 
        (~project_data['Date of TPT Start (yyyy-mm-dd)'].isna()) & (project_data['Client Verification Outcome'].isin(['valid', '']))]
    project_issues['DateTPTcompletion_lt_DateTPTStart'] = DateTPTcompletion_lt_DateTPTStart.shape[0]
    line_lists[f"{project_name}_DateTPTcompletion_lt_DateTPTStart"] = DateTPTcompletion_lt_DateTPTStart


    # Condition :Date of TPT Completion with blank TPT Completion status
    DateTPTcompletion_without_TPTCompletionStatus = project_data[
        (~project_data['TPT Completion date (yyyy-mm-dd)'].isna()) &
        (project_data['TPT Completion status'].isna()) &
        (project_data['Client Verification Outcome'].isin(['valid', ''])) ]
    project_issues['DateTPTcompletion_without_TPTCompletionStatus'] = DateTPTcompletion_without_TPTCompletionStatus.shape[0]
    line_lists[f"{project_name}_DateTPTcompletion_without_TPTCompletionStatus"] = DateTPTcompletion_without_TPTCompletionStatus

    # Condition : Date of Cervical Cancer Screening < ART start Date
    DateCXCA_Screening_lt_ARTStart = project_data[
        (project_data['Date of Cervical Cancer Screening (yyyy-mm-dd)'] < project_data['ART Start Date (yyyy-mm-dd)']) & (project_data['Client Verification Outcome'].isin(['valid', '']))]
    project_issues['DateCXCA_Screening_lt_ARTStart'] = DateCXCA_Screening_lt_ARTStart.shape[0]
    line_lists[f"{project_name}_DateCXCA_Screening_lt_ARTStart"] = DateCXCA_Screening_lt_ARTStart

     
    # Condition :Date of Cervical Cancer Screening with blank Cervical Cancer Screening Type
    CXCA_Screening_blank_Screentype = project_data[
        (~project_data['Date of Cervical Cancer Screening (yyyy-mm-dd)'].isna()) &
        (project_data['Cervical Cancer Screening Type'].isna()) &
        (project_data['Client Verification Outcome'].isin(['valid', ''])) ]
    project_issues['CXCA_Screening_blank_Screentype '] = CXCA_Screening_blank_Screentype .shape[0]
    line_lists[f"{project_name}_CXCA_Screening_blank_Screentype"] = CXCA_Screening_blank_Screentype

    # Condition :Date of Cervical Cancer Screening with blank Cervical Cancer Screening Method
    CXCA_Screening_blank_Screenmethod = project_data[
        (~project_data['Date of Cervical Cancer Screening (yyyy-mm-dd)'].isna()) &
        (project_data['Cervical Cancer Screening Method'].isna()) &
        (project_data['Client Verification Outcome'].isin(['valid', ''])) ]
    project_issues['CXCA_Screening_blank_Screenmethod'] = CXCA_Screening_blank_Screenmethod.shape[0]
    line_lists[f"{project_name}_CXCA_Screening_blank_Screenmethod"] = CXCA_Screening_blank_Screenmethod


    # Condition :Result of Cervical Cancer Screening = 'positive' with blank Date of Precancerous Lesions Treatment
    CXCA_Screening_blank_Treatmentdate = project_data[
        ...
        (project_data['Client Verification Outcome'].isin(['valid', ''])) ]
    project_issues['CXCA_Screening_blank_Treatmentdate'] = CXCA_Screening_blank_Treatmentdate.shape[0]
    line_lists[f"{project_name}_CXCA_Screening_blank_Treatmentdate"] = CXCA_Screening_blank_Treatmentdate
